...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <View style={[sb.badge, { backgroundColor: color + "18", borderColor: color + "40" }]}>
      ...
      <Text style={[sb.label, { color }]}>{STATUS_LABEL[status] ?? status}</Text>
    </View>
  );
}
    ...
    <TouchableOpacity style={lc.card} activeOpacity={0.75} onPress={onPress}>
      ...
      <View style={lc.statsRow}>
        <View style={lc.stat}>
          ...
          {expected > 0 ? <Text style={lc.statMuted}>/ {expected}m</Text> : null}
        </View>
        {totalComponents > 0 ? (
          <View style={lc.stat}>
            <Box size={12} color={INDIGO} />
            <Text style={[lc.statVal, { color: INDIGO }]}>{totalComponents} items</Text>
          </View>
        ) : null}
        <View style={lc.stat}>
          <User size={12} color={MUTED} />
          <Text style={lc.statMuted} numberOfLines={1}>{linemanName(log)}</Text>
        </View>
      </View>

      {/* Date */}
      <Text style={lc.date}>{log.end_time ? fmtDate(log.end_time) : fmtDate(log.start_time)}</Text>
    </TouchableOpacity>
  );
}
...
const lc = StyleSheet.create({
  ...
  statsRow:  { flexDirection: "row", gap: 14, marginBottom: 8, flexWrap: "wrap" },
        ...
        <View style={dm.sheet}>
          ...
          <ScrollView contentContainerStyle={dm.body} showsVerticalScrollIndicator={false}>
            ...
            <View style={dm.card}>
              ...
              <View style={dm.row}>
                <View style={dm.kv}>
                  ...
                  <Text style={[dm.kvVal, { color: GREEN }]}>{log.actual_cable ?? 0}m</Text>
                </View>
                <View style={dm.kv}>
            ...
            <View style={dm.card}>
              ...
              {log.team ? <Text style={dm.teamLabel}>{log.team.name}</Text> : null}
            </View>
...
export default function LogsScreen() {
  ...
  const fetchLogs = useCallback(async () => {
    try {
      ...
      setError(e?.message ?? "Failed to load teardown logs.");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  return (
    <SafeAreaView style={s.root} edges={["top"]}>
      ...
      <View style={s.header}>
        ...
        <TouchableOpacity style={s.dailyBtn} onPress={() => router.push("/daily-report" as any)}>
      ...
      <View style={s.filterBar}>
        ...
        >
          <Text style={[s.filterChipText, filterMode === "all" && s.filterChipTextActive]}>All Logs</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[s.filterChip, filterMode === "mine" && s.filterChipActive]}
          onPress={() => setFilterMode("mine")}
        >
          <Text style={[s.filterChipText, filterMode === "mine" && s.filterChipTextActive]}>My Reports</Text>
        </TouchableOpacity>
      </View>
      ...
      {loading ? (
        ...
        <View style={s.centered}>
          ...
          <Text style={s.emptySub}>Submitted teardowns will appear here</Text>
        </View>
      ) : (
        <FlatList
          data={filteredLogs}
...
const s = StyleSheet.create({
  ...
  dailyBtn: {
    ...
    backgroundColor: SLATE, paddingHorizontal: 14, paddingVertical: 9, borderRadius: 12,
  },
  dailyBtnText: { fontSize: 13, fontWeight: "700", color: "#fff" },

  filterBar: { flexDirection: "row", gap: 8, paddingHorizontal: 20, paddingBottom: 12 },
  filterChip: {
  ...
  errorText:   { fontSize: 14, color: "#DC2626", textAlign: "center", marginTop: 12, fontWeight: "600" },
  retryBtn:    { flexDirection: "row", alignItems: "center", gap: 6, marginTop: 16, backgroundColor: GREEN_L, paddingHorizontal: 16, paddingVertical: 10, borderRadius: 12 },
  retryBtnText:{ fontSize: 14, fontWeight: "700", color: GREEN },
  emptyTitle:  { fontSize: 18, fontWeight: "800", color: SLATE, marginTop: 16 },
  emptySub:    { fontSize: 13, color: MUTED, fontWeight: "500", marginTop: 6, textAlign: "center" },
  ...
  bannerNum:  { fontSize: 22, fontWeight: "900", color: SLATE },
  bannerLbl:  { fontSize: 10, fontWeight: "600", color: MUTED, marginTop: 2, textAlign: "center" },
