...
async function parseJson(res: Response): Promise<any> {
  ...
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error("Response parse timeout")), 10_000);
    res.text().then(text => {
      clearTimeout(timer);
      try { resolve(JSON.parse(text)); }
      catch { resolve({ message: text }); }
...
async function request<T>(
  ...
  const res = await fetchWithTimeout(`${BASE_URL}${path}`, { ...options, headers }, timeout);
  ...
  if (!res.ok) {
    const fieldMessages = data.errors
      ...
      : null;
    const err: any = new Error(fieldMessages || data.message || `HTTP ${res.status}`);
